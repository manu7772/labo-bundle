        ...
        switch ($pageName) {
            ...
            case Crud::PAGE_EDIT:
                ...
                switch (true) {
                    ...
                    case $twigfile === true:
                        yield ChoiceField::new('twigfile', 'Modèle de mise en page')
                            ->setChoices(static fn (FinalWebsectionInterface $websection): array => $websection->getTwigfileChoices() ?: [])
                            ->escapeHtml(false)
                ...
                yield ChoiceField::new('sectiontype', 'Type de section')
                    ->setChoices(static fn (FinalWebsectionInterface $websection): array => $websection->getTwigfileMetadata()->getSectiontypeChoices() ?: [])->setDisabled(!$this->isGranted('ROLE_SUPER_ADMIN'))
                    ->setFormTypeOption('by_reference', false)
                    ->setColumns(4);
